DROP FUNCTION IF EXISTS qa_set_candidate_relations;

CREATE OR REPLACE FUNCTION qa_set_candidate_relations
(
...
BEGIN
	...
	SELECT 	vr_application_id, 
			...
			FALSE
	FROM "data" AS d
		LEFT JOIN qa_candidate_relations AS cr
		ON cr.application_id = vr_application_id AND cr.workflow_id = vr_workflow_id AND 
			(cr.node_type_id = d.node_type_id OR cr.node_id = d.node_id)
	WHERE cr.workflow_id IS NULL;
