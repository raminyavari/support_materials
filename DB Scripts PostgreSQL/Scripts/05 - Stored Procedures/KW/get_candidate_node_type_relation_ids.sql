DROP FUNCTION IF EXISTS kw_get_candidate_node_type_relation_ids;

CREATE OR REPLACE FUNCTION kw_get_candidate_node_type_relation_ids
(
...
BEGIN
	...
	WHERE cr.application_id = vr_application_id AND 
		...
		cr.node_type_id IS NOT NULL AND cr.deleted = FALSE;
END;
$$ LANGUAGE plpgsql;
