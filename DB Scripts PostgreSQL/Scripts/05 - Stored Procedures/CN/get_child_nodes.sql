DROP FUNCTION IF EXISTS cn_get_child_nodes;

CREATE OR REPLACE FUNCTION cn_get_child_nodes
(
...
BEGIN
	...
	vr_parent_type_id := cn_fn_get_parent_relation_type_id(vr_application_id);
	...
	vr_ret_ids := ARRAY(
		...
		WHERE nr.application_id = vr_application_id AND 
			nr.source_node_id = vr_node_id AND nr.property_id = vr_parent_type_id AND
