DROP FUNCTION IF EXISTS cn_get_default_related_nodes;

CREATE OR REPLACE FUNCTION cn_get_default_related_nodes
(
...
BEGIN
	...
	vr_related_type_id := cn_fn_get_related_relation_type_id(vr_application_id);
	...
	vr_ret_ids := ARRAY(
		...
		WHERE nr.application_id = vr_application_id AND 
			nr.source_node_id = vr_node_id AND nr.property_id = vr_related_type_id AND
