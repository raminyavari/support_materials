DROP FUNCTION IF EXISTS cn_get_brother_nodes;

CREATE OR REPLACE FUNCTION cn_get_brother_nodes
(
...
BEGIN
	...
	vr_ret_ids := ARRAY(
		SELECT nr.source_node_id
		FROM UNNEST(vr_parent_node_ids) AS "p"
			INNER JOIN cn_node_relations AS nr
			ON nr.destination_node_id = "p"
			INNER JOIN cn_nodes AS nodes
			ON nodes.application_id = vr_application_id AND nodes.node_id = nr.source_node_id
		WHERE nr.application_id = vr_application_id AND 
			nr.property_id = vr_child_type_id AND nr.deleted = FALSE AND 
			nodes.node_type_id = vr_node_type_id AND nodes.deleted = FALSE
	);
