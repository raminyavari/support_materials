DROP FUNCTION IF EXISTS cn_get_expertise_domains;

CREATE OR REPLACE FUNCTION cn_get_expertise_domains
(
...
BEGIN
	...
	WITH "data" AS (
		SELECT 	ROW_NUMBER() OVER (
				...
				nd.node_id
		FROM cn_view_experts AS ex
			INNER JOIN cn_nodes AS nd
			ON nd.application_id = vr_application_id AND nd.node_id = ex.node_id AND (
