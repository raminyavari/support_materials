DROP FUNCTION IF EXISTS cn_get_child_hierarchy_member_ids;

CREATE OR REPLACE FUNCTION cn_get_child_hierarchy_member_ids
(
	...
	"data" AS (
		...
		FROM (
				...
				FROM ids AS x
					...
					ON nm.node_id = x.node_id
				WHERE nm.application_id = vr_application_id AND nm.is_pending = FALSE
			) AS u
