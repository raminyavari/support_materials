DROP FUNCTION IF EXISTS cn_is_document;

CREATE OR REPLACE FUNCTION cn_is_document
(
...
BEGIN
	...
	IF vr_is_document IS NULL THEN
		RETURN COALESCE((
			SELECT s.is_document
	...
	ELSE
		...
		SET is_document = vr_is_document
		WHERE s.application_id = vr_application_id AND s.node_type_id = vr_node_type_id_or_node_id;
