DROP FUNCTION IF EXISTS cn_related_nodes_select_disabled;

CREATE OR REPLACE FUNCTION cn_related_nodes_select_disabled
(
...
BEGIN
	...
	IF vr_value IS NULL THEN
		...
		WHERE s.application_id = vr_application_id AND s.disable_related_nodes_select = TRUE;
	ELSE
		UPDATE cn_services
		SET disable_related_nodes_select = vr_value
		FROM UNNEST(vr_node_type_ids) AS nt
			INNER JOIN cn_services AS s
			ON s.node_type_id = nt.first_value
