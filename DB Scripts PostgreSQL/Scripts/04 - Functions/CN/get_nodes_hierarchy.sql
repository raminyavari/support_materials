...
BEGIN
	...
	AS 
	(
		SELECT nd.node_id AS "id", nd.node_type_id AS type_id, nd.parent_node_id AS parent_id, 0::INTEGER AS "level", nd.name
		...
		SELECT node.node_id AS "id", node.node_type_id, node.parent_node_id AS parent_id, hr."level" + 1, node.name
		FROM cn_nodes AS node
			INNER JOIN "hierarchy" AS hr
			ON hr.parent_id = node.node_id
		WHERE node.application_id = vr_application_id AND node.node_type_id = hr.type_id AND
