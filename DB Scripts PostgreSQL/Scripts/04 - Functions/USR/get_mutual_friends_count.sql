...
BEGIN
	RETURN QUERY
	SELECT o AS user_id, COUNT(f.friend_id) AS mutuals_count
	FROM usr_view_friends AS f
		INNER JOIN UNNEST(vr_other_user_ids) AS o
		INNER JOIN usr_view_friends AS f2
		ON f2.application_id = vr_application_id AND f2.user_id = o
