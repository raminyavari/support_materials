DROP VIEW IF EXISTS cn_view_list_members;

CREATE VIEW cn_view_list_members
AS
...
FROM    cn_lists AS l
		...
		ON nm.application_id = nd.application_id AND nm.node_id = nd.node_id
		INNER JOIN rv_membership AS m
		ON m.user_id = nm.user_id
WHERE	ln.deleted = FALSE AND nd.deleted = FALSE AND 
		nm.status = N'Accepted' AND nm.deleted = FALSE AND m.is_approved = TRUE;