DROP FUNCTION IF EXISTS cn_p_make_correlation;

CREATE OR REPLACE FUNCTION cn_p_make_correlation
(
...
BEGIN
	...
	vr_relations := ARRAY(
		SELECT ROW(r.first_value, r.second_value, vr_related_relation_type_id)
		FROM UNNEST(vr_pair_node_ids) AS r
		
		UNION ALL
		
		SELECT ROW(r.second_value, r.first_value, vr_related_relation_type_id)
		FROM UNNEST(vr_pair_node_ids) AS r
	);
